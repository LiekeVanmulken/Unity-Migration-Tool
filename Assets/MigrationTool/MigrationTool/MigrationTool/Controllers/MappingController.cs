...
namespace migrationtool.controllers
{
    public class MappingController
    {
        ...
        /// <param name="newIDs"></param>
        /// <returns></returns>
        // private List<ScriptMapping> MapAllClasses(List<ClassModel> oldIDs, List<ClassModel> newIDs)
        // {
        ...
        //     {
        //         if (oldID.FullName.StartsWith("YamlDotNet.") || oldID.FullName.StartsWith("migrationtool."))
        //         {
        //             continue;
        //         }
        //
        //         if (scriptMappings.FirstOrDefault(_mapping => _mapping.oldClassModel.FullName == oldID.FullName) !=
        //             null || classesToMap.FirstOrDefault(_mapping => _mapping.FullName == oldID.FullName) != null)
        //         {
        //             Debug.LogError("Already mapped : " + oldID.FullName);
        //             continue;
        //         }
        //
        ...
        // }
        private List<ScriptMapping> MapAllClassesRecursiveParent(List<ClassModel> oldIDs, List<ClassModel> newIDs)
        {
            List<ScriptMapping> scriptMappings = new List<ScriptMapping>();
            List<ClassModel> classesToMap = new List<ClassModel>();
            foreach (ClassModel oldID in oldIDs)
            {
                MapAllClassesRecursive(oldID, newIDs, ref scriptMappings, classesToMap);
            }
            ...
            foreach (ScriptMapping _mapping in scriptMappings)
            {
                foreach (FieldModel _field in _mapping.oldClassModel.Fields)
                {
                    _field.Type = new ClassModel(_field.Type.FullName);
                }
                foreach (FieldModel _field in _mapping.newClassModel.Fields)
                {
                    _field.Type = new ClassModel(_field.Type.FullName);
                }
            }
        ...
        private List<ScriptMapping> MapAllClassesRecursive(ClassModel oldIDParent, List<ClassModel> newIDs,
            ref List<ScriptMapping> scriptMappings, List<ClassModel> classesToMap)
        {
            CheckClass(oldIDParent, ref scriptMappings, newIDs, classesToMap);
            foreach (FieldModel oldIDField in oldIDParent.Fields)
            {
                CheckClass(oldIDField.Type, ref scriptMappings, newIDs, classesToMap);
            }

            return scriptMappings;
        }

        private void CheckClass(ClassModel oldID, ref List<ScriptMapping> scriptMappings, List<ClassModel> newIDs,
            List<ClassModel> classesToMap)
        {
            if (oldID==null || oldID.FullName==null || oldID.FullName.StartsWith("YamlDotNet.") || oldID.FullName.StartsWith("migrationtool."))
            {
                return;
            }

            if (scriptMappings.FirstOrDefault(_mapping => _mapping.oldClassModel.FullName == oldID.FullName) != null ||
                classesToMap.FirstOrDefault(_mapping => _mapping.FullName == oldID.FullName) != null)
            {
                return;
            }
